...
| 1A.8 | 🟡 Media | **Devlog progetto** — diario di avanzamento a timeline verticale nella scheda progetto. Ogni voce: data, testo libero, foto opzionale, fase opzionale. Layout: linea verticale sinistra + card a destra in ordine cronologico inverso. Aggiunta via bottom sheet minimale (testo + foto, nessun wizard). DB: nuova tabella `project_logs` (id, project_id, text, photo_path, phase, created_at) → richiede schema v5 + migrazione. | ⬜ Da fare |
