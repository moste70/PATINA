...
| 3.2 | ✅ **AI Vision scansione istruzioni** — Claude Vision (Pro) identifica codici colore da foto del foglio istruzioni; utenti Free usano MLKit esistente. `scanManualColors` Firebase Function su `europe-west1`. Badge AI PRO nel tips sheet, stesso flusso UX per entrambi i path. |
...
| 3.4 | ✅ **Riconoscimento colore da foto** — viewer con pan/zoom reale (`InteractiveViewer` + cerchio overlay, stessa architettura di `PinViewerScreen`: gesture di tap e pannello non competono più per lo stesso puntatore) + campionamento pixel (dart:ui) + ΔE client-side contro inventario (top 2) e catalogo (top 2, con azione rapida "Aggiungi all'inventario"); foto da camera non salvata. Entry point: icona `colorize` in AppBar Ricette e Vernici. Nessuna chiamata AI — tutto offline. |
