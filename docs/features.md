...
| Pin su foto | (viewer foto fullscreen) | ✅ Implementato — tap apre context menu Colore/Nota; pin colore via `PaintPickerSheet` (catalogo con brand/linea/ricerca); pin nota con testo libero; chip esagonale per colore, icona build per nota; long-press apre tooltip overlay con swatch + dettagli + azioni modifica/elimina |
