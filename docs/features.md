...
| Lista della spesa | `/shopping` | ✅ Implementato — sezione "Vernici" unisce due fonti: vernici mancanti dalla palette kit di un progetto (non presenti in inventario, swipe per escludere/ripristinare, schema v7) e vernici aggiunte senza un progetto attivo (es. dal carrello nella schermata "rileva colore da foto" — schema v11, swipe per eliminare) — entrambe mostrate come `HexColorChip` con brand/codice; sezione "Altro" per voci manuali di testo libero (DB), swipe-to-delete; FAB per aggiunta manuale |
